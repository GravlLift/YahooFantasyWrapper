...
namespace System.Net.Http.Xml
{
    public sealed partial class XmlContent : HttpContent
    {
        ...
        private XmlContent(
            object? inputValue,
            Type inputType,
            MediaTypeHeaderValue? mediaType /*, XmlSerializerNamespaces? namespaces*/
        ) {
            if (inputType == null)
            {
        ...
        public static XmlContent Create<T>(
            T inputValue,
            MediaTypeHeaderValue? mediaType = null /*, XmlSerializerNamespaces? namespaces = null*/
        ) =>
            Create(
                inputValue,
                typeof(T),
        ...
        private async Task SerializeToStreamAsyncCore(
            ...
            {
                using TranscodingWriteStream transcodingStream = new TranscodingWriteStream(
                    targetStream,
                    targetEncoding
                );
