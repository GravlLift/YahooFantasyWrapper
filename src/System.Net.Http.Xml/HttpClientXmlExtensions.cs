...
namespace System.Net.Http.Xml
{
    ...
    {
        public static Task<object?> GetFromXmlAsync(
            ...
            Task<HttpResponseMessage> taskResponse = client.GetAsync(
                requestUri,
        ...
        public static Task<TValue?> GetFromXmlAsync<TValue>(
            this HttpClient client,
            string? requestUri,
            XmlSerializer? xmlSerializer = null,
            CancellationToken cancellationToken = default
        ...
        private static async Task<object?> GetFromXmlAsyncCore(
            ...
            return await response.Content!.ReadFromXmlAsync(type, xmlSerializer)
                .ConfigureAwait(false);
        }

        private static async Task<T?> GetFromXmlAsyncCore<T>(
            Task<HttpResponseMessage> taskResponse,
            XmlSerializer? xmlSerializer
        ) {
            using HttpResponseMessage response = await taskResponse.ConfigureAwait(false);
